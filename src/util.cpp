...
const char * const BITCOIN_CONF_FILENAME = "donu.conf";
const char * const BITCOIN_PID_FILENAME = "donu.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "donud -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\donu
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\donu
    // Mac: ~/Library/Application Support/donu
    // Unix: ~/.donu
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "donu";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/donu";
#else
    // Unix
    return pathRet / ".donu";
#endif
