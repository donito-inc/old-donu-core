...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " donu-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  donu-tx [options] <hex-tx> [commands]  Update hex-encoded donu transaction\n" +
            "or:     donu-tx [options] -create [commands]   Create hex-encoded donu transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded donu transaction
