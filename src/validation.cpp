...
#if defined(NDEBUG)
# error "DONU cannot be compiled without assertions."
#endif
                                     ...
                                     bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept)
{
...

    {
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s DONU additional fees, %d delta bytes\n",
...
{
    // Previous Neos chain supply + additional funding to cover any
    // potential deposits while Bittrex keeps their wallet open.
    if (nPrevHeight == 2)      return 4463497 * COIN;

    // Platform funding
    if (nPrevHeight == 4)      return 2000000 * COIN;
...
/*
                if (tx.IsCoinStake()) {
                  CAmount inputSize = coin.out.nValue;
                  if (inputSize < (Params().GetConsensus().nStakeMinSize * COIN)) {
		    LogPrintf("CheckInputs(): input size: %ld < minimum (%ld)\n", inputSize, Params().GetConsensus().nStakeMinSize * COIN);
                    return state.Invalid(error("CheckInputs(): tried to stake with smaller than minimum amount allowed"));
                  }
            	}
*/
                               ...
                               CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Special case for 'crisis/damage control'
    if (sporkManager.IsSporkActive(Spork::SPORK_15_POS_DISABLED) && block.IsProofOfStake())
        return state.DoS(0, error("ConnectBlock(): PoS blocks temporarily not accepted"), REJECT_INVALID, "PoS-halted");
    ...
    CAmount devFee = Params().GetEmissionsAmount();
    CAmount nValueOut = 0;
    CAmount nValueIn = 0;
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
 	...
 	for (unsigned int j = 0; j < tx.vout.size(); j++) {
          if (tx.vout[j].scriptPubKey == PlatformScript()) {
    ...
    if (!IsBlockPayeeValid(coinbaseTransaction, pindex->nHeight, expectedReward, pindex->nMint)) {
        return state.DoS(0, error("ConnectBlock(DONU): couldn't find masternode or superblock payments"),
                         REJECT_INVALID, "bad-cb-payee");
    }

    // END DONU

    if (chainActive.Height() > 0 && !devFeePaid && !sporkManager.IsSporkActive(Spork::SPORK_15_POS_DISABLED)) {
        return state.DoS(100, error("ConnectBlock() : coinbase does not pay to the platform"));
    }
