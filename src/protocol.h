...
/**
 * DONU protocol message types. When adding new message types, don't forget
...
enum ServiceFlags : uint64_t {
    ...
    // set by all DONU non pruned nodes, and is unset by SPV clients or other light clients.
    NODE_NETWORK = (1 << 0),
    // NODE_GETUTXO means the node is capable of responding to the getutxo protocol request.
    // DONU does not support this but a patch set called DONU XT does.
    // See BIP 64 for details on how this is implemented.
    NODE_GETUTXO = (1 << 1),
    // NODE_BLOOM means the node is capable and willing to handle bloom-filtered connections.
    // DONU nodes used to support this by default, without advertising this bit,
...
const uint32_t MSG_DONU_OFFSET = 1000;
...
enum GetDataMsg
{
    ...
    MSG_TXLOCK_REQUEST = MSG_DONU_OFFSET,
    MSG_TXLOCK_VOTE,
    MSG_SPORK,
