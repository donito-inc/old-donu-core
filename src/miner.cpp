...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(CWallet *wallet, const CScript &scriptPubKeyIn, bool fProofOfStake, bool fMineWitnessTx)
{
    ...
    if(fProofOfStake && !sporkManager.IsSporkActive(Spork::SPORK_15_POS_DISABLED))
    {
        ...
        if (nSearchTime >= nLastCoinStakeSearchTime) {
                                        ...
                                        vwtxPrev, fIncludeWitness))
            {
                ...
                coinstakeTx.vout.push_back(CTxOut(Params().GetEmissionsAmount(), PlatformScript()));
	...
	if (!mnpayments.GetBlockPayee(nHeight, payee)) {
	    hasPayment = false;
            LogPrintf("CreateNewBlock() : !mnpayments.GetBlockPayee() \n");
	    int nCount = 0;
	    masternode_info_t mnInfo;
	    if(!mnodeman.GetNextMasternodeInQueueForPayment(pindexPrev->nHeight + 1, true, nCount, mnInfo)) {
                LogPrintf("CreateNewBlock() : !mnodeman.GetNextMasternodeInQueueForPayment \n");
	...
	if (hasPayment) {
            LogPrintf("CreateNewBlock() : hasPayment \n");
...
void static DONUMiner(const CChainParams& chainparams, CConnman& connman, CWallet* pwallet, bool fProofOfStake)
{
    ...
    while (true) {
        try {
            ...
            LogPrintf("DONUMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
            ...
            if (fProofOfStake)
            {
                ...
                if (!SignBlock(*pblock, *pwallet)) {
                    LogPrintf("DONUMiner(): Signing new block failed \n");
        ...
        {
            LogPrintf("DONUMiner -- terminated\n");
            throw;
        }
...
void GenerateDONUs(bool fGenerate,
                  int nThreads,
                  ...
                  CConnman &connman)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DONUMiner, boost::cref(chainparams), boost::ref(connman), GetWallets().front(), false));
}
...
void ThreadStakeMinter(const CChainParams &chainparams, CConnman &connman, CWallet *pwallet)
{
    ...
    try {
        DONUMiner(chainparams, connman, pwallet, true);
        boost::this_thread::interruption_point();
