...
#include <qt/donuaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a DONU address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new DONUAddressEntryValidator(parent));
    widget->setCheckValidator(new DONUAddressCheckValidator(parent));
}
...
bool parseDONUURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!DONUUnits::parse(DONUUnits::DONU, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseDONUURI(QString uri, SendCoinsRecipient *out)
{
    ...
    return parseDONUURI(uriInstance, out);
}
...
QString formatDONUURI(const SendCoinsRecipient &info)
{
    ...
    if (info.amount)
    {
        ret += QString("?amount=%1").arg(DONUUnits::format(DONUUnits::DONU, info.amount, false, DONUUnits::separatorNever));
        paramCount++;
    }
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "DONU.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "DONU (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("DONU (%s).lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=DONU\n";
        else
            optionFile << strprintf("Name=DONU (%s)\n", chain);
