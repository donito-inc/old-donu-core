...
*
*   Why is this needed?
*   - In DONU some blocks are superblocks, which output much higher amounts of coins
...
{
    if(fLiteMode) return; // disable all DONU specific functionality
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransactionRef& txNew) const
{
    ...
    LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f DONU\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
...
bool CMasternodePaymentVote::IsValid(CNode* pnode, int nValidationHeight, std::string& strError, CConnman& connman)
{
    ...
    if((nBlockHeight - 101) > 0 && !mnodeman.GetMasternodeRank(vinMasternode.prevout, nRank, nBlockHeight - 101, nMinRequiredProtocol)) {
...
void AdjustMasternodePayment(CMutableTransaction &tx, const CTxOut &txoutMasternodePayment)
{
    ...
    LogPrintf("AdjustMasternodePayment(): Tx: %s", tx.ToString().c_str());
}
