 ...
 * between donud, and donu-qt and non-server tools.
*/
...
{
    return CopyrightHolders(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           (strprintf(_("Copyright (C) 2014-%i The Dash Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           (strprintf(_("Copyright (C) 2011-%i Peercoin Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           strprintf("Copyright (C) 2019 barrystyle") + "\n" +
           "\n" +
           strprintf("Copyright (C) 2019 The Donu Team") + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that DONU is running in a usable environment with all
...
{
    // Make sure only a single DONU process is using the data directory.
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if donu is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if donu is started while in a temporary directory.\n",
    ...
    // ********************************************************* Step 11c: update block tip in DONU modules

    // force UpdatedBlockTip to initialize nCachedBlockHeight for DS, MN payments and budgets
    // but don't call it directly to prevent triggering of other listeners like zmq etc.
    pdsNotificationInterface->InitializeCurrentBlockTip();

    // ********************************************************* Step 11d: start thread for donu extensions
