...
static UniValue masternodelist(const JSONRPCRequest& request)
{
                ...
                strMode != "rank" && strMode != "status"))
    {
        throw std::runtime_error(
                ...
                "  payee          - Print DONU address associated with a masternode (can be additionally filtered,\n"
                "                   partial match)\n"
        ...
        for (auto& mnpair : mapMasternodes) {
            ...
            if (strMode == "activeseconds") {
                ...
                streamFull << std::setw(18) <<
                               mn.GetStatus() << " " <<
                               mn.nProtocolVersion << " " <<
                               CDONUAddress(mn.pubKeyCollateralAddress.GetID()).ToString() << " " <<
                ...
                CDONUAddress address(mn.pubKeyCollateralAddress.GetID());
...
//            return mnodeman.CountEnabled(MIN_PRIVATESEND_PEER_PROTO_VERSION);
    ...
    if (strCommand == "current" || strCommand == "winner")
    {
        ...
        obj.push_back(Pair("outpoint",      mnInfo.vin.prevout.ToString()));
        obj.push_back(Pair("payee",         CDONUAddress(mnInfo.pubKeyCollateralAddress.GetID()).ToString()));
    ...
    if (strCommand == "status")
    {
        ...
        if(mnodeman.Get(activeMasternode.outpoint, mn)) {
            mnObj.push_back(Pair("payee", CDONUAddress(mn.pubKeyCollateralAddress.GetID()).ToString()));
        }
    ...
    if (strCommand == "decode")
    {
        ...
        for(CMasternodeBroadcast& mnb : vecMnb) {
            ...
            if(mnb.CheckSignature(nDos)) {
                ...
                resultObj.push_back(Pair("pubKeyCollateralAddress", CDONUAddress(mnb.pubKeyCollateralAddress.GetID()).ToString()));
                resultObj.push_back(Pair("pubKeyMasternode", CDONUAddress(mnb.pubKeyMasternode.GetID()).ToString()));
