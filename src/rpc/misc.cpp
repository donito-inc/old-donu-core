...
static UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturn information about the given donu address.\n"
            "DEPRECATION WARNING: Parts of this command have been deprecated and moved to getaddressinfo. Clients must\n"
            "transition to using getaddressinfo to access this information before upgrading to v0.18. The following deprecated\n"
            "fields have moved to getaddressinfo and will only be shown here with -deprecatedrpc=validateaddress: ismine, iswatchonly,\n"
            "script, hex, pubkeys, sigsrequired, pubkey, addresses, embedded, iscompressed, account, timestamp, hdkeypath, kdmasterkeyid.\n"
            "\nArguments:\n"
            "1. \"address\"                    (string, required) The donu address to validate\n"
            "\nResult:\n"
            "{\n"
...
static UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The donu address to use for the signature.\n"
            ...
            "\nAs json rpc\n"
            + HelpExampleRpc("verifymessage", "\"NNwUD95yCqxYpWWL2fZqfRTNQreqTffRCY\", \"signature\", \"my message\"")
        );
...
static UniValue echo(const JSONRPCRequest& request)
{
    if (request.fHelp)
        throw std::runtime_error(
            ...
            "donu-cli and the GUI. There is no server-side difference."
        );
...
static UniValue getinfo_deprecated(const JSONRPCRequest& request)
{
    throw JSONRPCError(RPC_METHOD_NOT_FOUND,
        ...
        "\ndonu-cli has the option -getinfo to collect and format these in the old format."
    );
}
...
UniValue getstakingstatus(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"enoughbalance\": true|false,        (boolean) if available coins are greater than minimum stake input size\n"
    ...
    if (pwalletMain) {
        ...
        obj.push_back(Pair("enoughbalance", pwalletMain->GetBalance() >= Params().GetConsensus().nStakeMinSize * COIN));
