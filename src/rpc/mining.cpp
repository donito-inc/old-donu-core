...
static UniValue generatetoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "2. address      (string, required) The address to send the newly generated donu to.\n"
...
// NOTE: Unlike wallet RPC (which use DONU values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
...
static UniValue getblocktemplate(const JSONRPCRequest& request)
{
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0)
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "DONU is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "DONU is downloading blocks...");
...
{
    throw JSONRPCError(RPC_METHOD_DEPRECATED, "estimatefee was removed in v1.0.\n"
...
static UniValue setgenerate(const JSONRPCRequest& request)
{
    ...
    GenerateDONUs(fGenerate, nGenProcLimit, Params(), *g_connman);
