...
{
    int64_t nCollateralAmount = Params().getMasternodeCollateral();
...
bool CWallet::MintableCoins()
{
    ...
    if(nBalance < Params().GetConsensus().nStakeMinSize * COIN) {
      LogPrintf("Balance %ld lower than minimum stake input size: %ld", nBalance,Params().GetConsensus().nStakeMinSize * COIN);
      return false;
    }
...
bool CWallet::SelectStakeCoins(StakeCoinsSet &setCoins, CAmount nTargetAmount, bool fSelectWitness, const CScript &scriptFilterPubKey) const
{
    ...
    for (const COutput& out : vCoins) {
        ...
        if(out.tx->tx->vout[out.i].nValue < Params().GetConsensus().nStakeMinSize)
            continue;
        ...
        if(!boost::get<CKeyID>(&dest) && !boost::get<WitnessV0KeyHash>(&dest) && !boost::get<CScriptID>(&dest))
            continue;
        ...
        bool isMined = out.tx->tx->IsCoinStake() || out.tx->tx->IsCoinBase();
        if (out.nDepth < (isMined ? Params().GetConsensus().nCoinbaseMaturity : Params().GetConsensus().nTransactionMaturity))
            continue;
                                ...
                                int& nChangePosInOut, std::string& strFailReason, const CCoinControl& coin_control, bool sign, OnTransactionToBeSigned onTxToBeSigned)
{
    ...
    int nBytes;
    {
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            // change transaction isn't always pay-to-donu-address
            CScript scriptChange;
